...
The release gate and publisher are implemented. The recurring producer is a separate operational dependency; its runtime is pending the user's choice of always-on server/API usage or a running Mac/Codex app. Do not claim that the daily producer is enabled solely because the time gate exists.
