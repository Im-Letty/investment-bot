...
(function(root,factory){
  ...
  function start(w){
    ...
    function init(){
      ...
      let state={status:'idle',items:[]},composing=false,opened=false,returnFocus=null,returnY=0,homeScroll=0;
      ...
      function renderSaved(){
        const current=store.list();count.textContent=current.length+' / '+LIMIT;saved.replaceChildren();
        ...
        current.forEach(key=>{const row=el('li','sw-row');row.appendChild(company({symbol:key,name:store.name(key)}));const button=el('button','sw-remove','削除');button.type='button';button.setAttribute('aria-label',store.name(key)+'をお気に入りから削除');button.addEventListener('click',async()=>{
          button.disabled=true;try{await store.remove(key);feedback('お気に入りから削除しました。');renderSaved();renderResults(state);notify();const next=saved.querySelector('button');(next||input).focus({preventScroll:true});}catch(e){feedback('削除できませんでした。もう一度お試しください。',true);button.disabled=false;}
      ...
      w.openStockWatchManager=async function(){
        if(!opened){returnFocus=doc.activeElement;returnY=w.scrollY||0;const home=doc.getElementById('morning-section');homeScroll=home?home.scrollTop:0;}
        opened=true;
        // The home is a fixed scrolling view whose display rule uses !important.
        doc.body.classList.remove('view-asaletter');
        ['lp-section','sim-section','morning-section','morning-news-section'].forEach(id=>{const node=doc.getElementById(id);if(node)node.style.display='none';});
        manager.style.display='block';w.scrollTo({top:0,behavior:'instant'});input.focus({preventScroll:true});
      ...
      w.closeStockWatchManager=function(){
        search.cancel();opened=false;manager.style.display='none';
        const home=doc.getElementById('morning-section'),news=doc.getElementById('morning-news-section');if(home)home.style.display='block';if(news)news.style.display='';doc.body.classList.add('view-asaletter');
        notify();if(w.__knSetSub)w.__knSetSub('watch');w.scrollTo({top:returnY,behavior:'instant'});
      ...
      doc.addEventListener('click',event=>{if(opened&&event.target.closest&&event.target.closest('#knBottomNav .bn-item'))w.closeStockWatchManager();},true);
      doc.addEventListener('keydown',event=>{if(opened&&event.key==='Escape'&&!composing)w.closeStockWatchManager();});
