...
(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;if(root&&root.document)api.start(root);})(typeof window==='undefined'?null:window,function(){
  ...
  function start(w){
    function init(){
      ...
      const filters=el('div','dc-filters');filters.setAttribute('aria-label','表示する銘柄');refs.all=button('配当','dc-filter',()=>setScope('all'));refs.favorites=button('お気に入り','dc-filter',()=>setScope('favorites'));refs.manage=button('銘柄を追加','dc-manage',()=>{if(w.openStockWatchManager)w.openStockWatchManager();});refs.benefits=button('株主優待','dc-filter',()=>setScope('benefits'));append(filters,refs.all,refs.benefits,refs.favorites,refs.manage);
      ...
      const layout=el('div','dc-layout'),calendarPanel=el('section','dc-calendar-panel'),agendaPanel=el('section','dc-agenda-panel');calendarPanel.setAttribute('aria-labelledby','dc-month-title');agendaPanel.setAttribute('aria-labelledby','dc-agenda-title');append(calendarPanel,weekdays,refs.grid,statusRow);append(agendaPanel,listHead,refs.list,refs.empty,refs.more,refs.plans);append(layout,calendarPanel,agendaPanel);const dividendPane=el('div','dc-dividend-pane'),benefitsPane=el('div','dc-benefits-pane');benefitsPane.hidden=true;append(dividendPane,refs.search,companyResults,navigation,layout,refs.coverage,refs.migration,legend);append(mount,filters,dividendPane,benefitsPane);let benefitsView=null;
      ...
      function syncPrices(){
        ...
        const active=!doc.hidden&&!dividendPane.hidden&&priceInView&&mount.getClientRects().length>0;
      ...
      function setScope(value){scopeChosen=true;dividendPane.hidden=value==='benefits';benefitsPane.hidden=value!=='benefits';refs.benefits.setAttribute('aria-pressed',String(value==='benefits'));if(value==='benefits'){syncPrices();refs.all.setAttribute('aria-pressed','false');refs.favorites.setAttribute('aria-pressed','false');if(!benefitsView&&w.KNBenefits)benefitsView=w.KNBenefits.create(w,benefitsPane,{today,shift,bounds,calendarDays});return;}if(scope===value){render();return;}scope=value;selectedDate=null;autoDate=true;limit=monthLimit=5;polls=0;refresh(true);}
