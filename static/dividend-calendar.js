...
(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;if(root&&root.document)api.start(root);})(typeof window==='undefined'?null:window,function(){
  'use strict';
  const FAVORITES='alert_watchlist_v1',LEGACY='myDividendStocks',MIGRATED='dividend_calendar_migrated_v1',NAMES='stock_watch_names_v1',CACHE='kn_dividend_calendar_v2:',KINDS={holding_deadline:'保有期限',payment:'配当の支払い',ex_dividend:'権利落ち日'};
  ...
  function start(w){
    function init(){
      ...
      refs.coverage=el('p','dc-coverage');refs.migration=el('p','dc-coverage');refs.universe=el('p','dc-universe');const legend=el('details','dc-help');append(legend,el('summary','','日付の見方'),el('p','','保有期限は、今回の配当の権利を得るため、この日の取引終了時まで株を持っておく日です。権利落ち日に買っても、今回の配当の権利は得られません。配当の支払いとは別の日です。'),refs.universe);
