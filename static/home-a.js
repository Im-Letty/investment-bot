...
(function() {
  ...
  async function boot() {
    ...
    var masthead=document.createElement('div');masthead.className='kn-a-masthead';masthead.innerHTML='<div class="kn-a-top-row"><button type="button" id="knHomeAMenu" aria-label="'+esc(text('menu'))+'">'+icon('menu')+'</button><span data-kn-home-text="morning">'+esc(text('morning'))+'</span><button type="button" id="knHomeAAccount" aria-label="'+esc(text('account'))+'">'+icon('user')+'</button></div><h1>経済NEWS</h1><p class="kn-a-wordmark-en">ECONOMIC INSIGHT</p><div id="knHomeADate"></div>';banner.appendChild(masthead);
