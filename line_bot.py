...
AI_ADVICE_MONTHLY_LIMIT = int(os.environ.get("AI_ADVICE_MONTHLY_LIMIT", "20"))_ai_advice_usage = {}@app.route("/api/ai-advice", methods=["POST"])def api_ai_advice():    try:        body = request.get_json(silent=True) or {}        user_key = body.get("user_id") or request.remote_addr or "anon"        lang = (body.get("lang") or "ja").lower()        if lang not in ("ja","en","ko","zh"):            lang = "ja"        summary = body.get("summary") or {}        now_month = datetime.now().strftime("%Y-%m")        rec = _ai_advice_usage.get(user_key, {"count": 0, "month": now_month})        if rec.get("month") != now_month:            rec = {"count": 0, "month": now_month}        if rec["count"] >= AI_ADVICE_MONTHLY_LIMIT:            return jsonify({"error": "monthly_limit", "limit": AI_ADVICE_MONTHLY_LIMIT, "used": rec["count"]}), 429        api_key = os.environ.get("GEMINI_API_KEY", "")        if not api_key:            return jsonify({"error": "GEMINI_API_KEY not set"}), 500        import google.generativeai as genai        genai.configure(api_key=api_key)        lang_names = {"ja":"Japanese","en":"English","ko":"Korean","zh":"Chinese"}        prompt = (            "You are a friendly personal finance advisor. Based on the user's household budget summary below, "            "give SHORT actionable advice. Respond ONLY in " + lang_names[lang] + ". "            "Return strict JSON with this exact shape: {\"summary\": string, \"trend\": string, \"savings_tips\": [string, string, string], \"investment_advice\": string, \"score\": number}. "            "score is 0-100 (healthy=high). Each string max 120 chars. No markdown, no code fences.

"            "Data (JSON):
" + json.dumps(summary, ensure_ascii=False)[:6000]        )        model = genai.GenerativeModel("gemini-1.5-flash")        resp = model.generate_content(prompt)        raw = (getattr(resp, "text", "") or "").strip()        t = raw        if t.startswith("```"):            t = t.strip("`")            if t.lower().startswith("json"):                t = t[4:].strip()        data = json.loads(t)        rec["count"] = rec.get("count", 0) + 1        _ai_advice_usage[user_key] = rec        gc.collect()        return jsonify({"ok": True, "data": data, "usage": rec["count"], "limit": AI_ADVICE_MONTHLY_LIMIT})    except json.JSONDecodeError:        return jsonify({"error": "ai_response_not_json"}), 502    except Exception as e:        return jsonify({"error": str(e)[:200]}), 500@app.route("/kakeibo")def kakeibo():
